  ...
  <md-app>
    <md-app-toolbar class="md-dense" md-elevation="0">
      ...
      <div class="md-toolbar-section-end">
        <md-button class="md-icon-button md-accent" v-if="wm.selected_window && wm.selected_window.standalone" @click="wm.closeWindow(wm.selected_window);" >
  ...
  <md-dialog class="plugin-dialog" :md-active.sync="showAddPluginDialog" :md-click-outside-to-close="false">
    ...
    <md-dialog-content>
      ...
      <md-card v-if="show_plugin_store">
        ...
        <md-card-content>
          <plugin-list @message="showMessage" v-if="pm" :plugin-manager="pm" :init-search="init_plugin_search" display="list" :plugins="pm.available_plugins" :workspace="pm.selected_workspace"></plugin-list>
        </md-card-content>
      </md-card>
    </md-dialog-content>
    <md-dialog-actions>
...
export default {
  ...
  mounted() {
    ...
    this.event_bus.$on('engine_connected', (engine)=>{
      if(this.pm)
      this.pm.reloadPythonPlugins(engine)
    })
  ...
  methods: {
            ...
            if(!this.showAddPluginDialog  && ps.length<=0){
              ...
              if(!this.showAddPluginDialog && (this.pm.registered.windows[pname] || pname.startsWith('imjoy/'))){
                try {
                  ...
                  c.tag = template.tag
                  c.fullscreen = this.$route.query.fullscreen || c.fullscreen || false
    ...
    connectPlugin(plugin){
      if(plugin._disconnected){
        if(plugin.type === 'native-python' && plugin.config.engine && !plugin.config.engine.connected){
